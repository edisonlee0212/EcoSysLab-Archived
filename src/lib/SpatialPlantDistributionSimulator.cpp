	...
	if(ImGui::Button("Create Trees"))
	{
		...
		for(const auto& plant : m_distribution.m_plants)
		{
			if(glm::abs(plant.m_position.x - offset.x) < range && glm::abs(plant.m_position.y - offset.y) < range
				&& m_treeDescriptors.size() > plant.m_parameterHandle && m_treeDescriptors.at(plant.m_parameterHandle).Get<TreeDescriptor>())
			{
				...
				tree->m_treeDescriptor = m_treeDescriptors.at(plant.m_parameterHandle);
