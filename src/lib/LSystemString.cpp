...
bool LSystemString::OnInspect(const std::shared_ptr<EditorLayer>& editorLayer) {
	bool changed = false;
	ImGui::Text(("Command Size: " + std::to_string(m_commands.size())).c_str());
	if (ImGui::DragFloat("Internode Length", &m_internodeLength)) changed = true;
	if (ImGui::DragFloat("Thickness Factor", &m_thicknessFactor)) changed = true;
	if (ImGui::DragFloat("End node thickness", &m_endNodeThickness)) changed = true;
	return changed;
}