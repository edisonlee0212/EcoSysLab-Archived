...
{
	if (m_boundary.m_points.size() <= 2)
	{
		m_boundaryValid = false;
		return;
	}
	for (int i = 0; i < m_boundary.m_points.size(); i++) {
		auto& pa = m_boundary.m_points[(i == 0 ? m_boundary.m_points.size() - 1 : i - 1)];
		auto& pb = m_boundary.m_points[i];
		for (int j = 0; j < m_boundary.m_points.size(); j++) {
			auto& pc = m_boundary.m_points[(j == 0 ? m_boundary.m_points.size() - 1 : j - 1)];
			auto& pd = m_boundary.m_points[j];
...
bool ProfileInfo::InBoundary(const glm::vec2& point) const
{
	...
	int windingNumber = 0;
	const auto size = m_boundary.m_points.size();
	if (size < 3) return false;
	for (int i = 0; i < size - 1; i++) {
		if (RayLineIntersect(point, point2, m_boundary.m_points[i], m_boundary.m_points[i + 1]) &&
			RayLineIntersect(point, point3, m_boundary.m_points[i], m_boundary.m_points[i + 1])) {
			windingNumber++;
		}
	}
	if (RayLineIntersect(point, point2, m_boundary.m_points[size - 1], m_boundary.m_points[0]) &&
		RayLineIntersect(point, point3, m_boundary.m_points[size - 1], m_boundary.m_points[0]))
		windingNumber++;
