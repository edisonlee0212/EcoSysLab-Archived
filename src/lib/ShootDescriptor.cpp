...
void ShootDescriptor::PrepareController(ShootGrowthController& shoot_growth_controller) const {
  shoot_growth_controller.m_baseInternodeCount = m_baseInternodeCount;
  shoot_growth_controller.m_breakingForce = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    if (m_branchStrength != 0.f && !internode.IsEndNode() && internode.info.thickness != 0.f &&
        internode.info.length != 0.f) {
      float branch_water_factor = 1.f;
      if (m_branchStrengthLightingThreshold != 0.f &&
          internode.data.max_descendant_light_intensity < m_branchStrengthLightingThreshold) {
        branch_water_factor = 1.f - m_branchStrengthLightingLoss;
      }

      return glm::pow(internode.info.thickness / m_endNodeThickness, m_branchStrengthThicknessFactor) *
             branch_water_factor * internode.data.strength * m_branchStrength;
    }
    return FLT_MAX;
  };
  shoot_growth_controller.m_sagging = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    float strength = m_endNodeThickness * internode.data.sagging_force * m_gravityBendingStrength /
                     glm::pow(internode.info.thickness / m_endNodeThickness, m_gravityBendingThicknessFactor);
  ...
  shoot_growth_controller.m_internodeGrowthRate = m_growthRate / m_internodeLength;
  ...
  shoot_growth_controller.m_rollAngle = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    ...
    value += m_rollAngleNoise2D.GetValue(glm::vec2(internode.GetHandle(), internode.info.root_distance));
    return value;
  };
  shoot_growth_controller.m_apicalAngle = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    if (m_straightTrunk != 0.f && internode.data.order == 0 && internode.info.root_distance < m_straightTrunk)
      return 0.f;
    ...
    value += m_apicalAngleNoise2D.GetValue(glm::vec2(internode.GetHandle(), internode.info.root_distance));
    return value;
  };
  ...
  shoot_growth_controller.m_internodeLength = m_internodeLength;
  shoot_growth_controller.m_internodeLengthThicknessFactor = m_internodeLengthThicknessFactor;
  shoot_growth_controller.m_endNodeThickness = m_endNodeThickness;
  shoot_growth_controller.m_thicknessAccumulationFactor = m_thicknessAccumulationFactor;
  shoot_growth_controller.m_thicknessAgeFactor = m_thicknessAgeFactor;
  shoot_growth_controller.m_internodeShadowFactor = m_internodeShadowFactor;

  shoot_growth_controller.m_lateralBudCount = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    if (m_maxOrder == -1 || internode.data.order < m_maxOrder) {
  ...
  shoot_growth_controller.m_lateralBudFlushingRate = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    float flushing_rate = m_lateralBudFlushingRate;
    if (internode.data.inhibitor_sink > 0.0f)
      flushing_rate *= glm::exp(-internode.data.inhibitor_sink);
    return flushing_rate;
  };
  shoot_growth_controller.m_apicalControl = m_apicalControl;
  shoot_growth_controller.m_rootDistanceControl = m_rootDistanceControl;
  shoot_growth_controller.m_heightControl = m_heightControl;

  shoot_growth_controller.m_apicalDominance = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    return m_apicalDominance * internode.data.light_intensity;
  };
  shoot_growth_controller.m_apicalDominanceLoss = m_apicalDominanceLoss;
  shoot_growth_controller.m_leaf = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    return internode.data.light_intensity > m_leafFlushingLightingRequirement;
  };
  ...
  shoot_growth_controller.m_fruit = [&](const SkeletonNode<InternodeGrowthData>& internode) {
    return internode.data.light_intensity > m_fruitFlushingLightingRequirement;
  };
...
void ShootDescriptor::Deserialize(const YAML::Node& in) {
  if (in["m_baseInternodeCount"])
    m_baseInternodeCount = in["m_baseInternodeCount"].as<int>();
  if (in["m_straightTrunk"])
    m_straightTrunk = in["m_straightTrunk"].as<float>();
  if (in["m_baseNodeApicalAngleMeanVariance"])
    m_baseNodeApicalAngleMeanVariance = in["m_baseNodeApicalAngleMeanVariance"].as<glm::vec2>();

  if (in["m_growthRate"])
    m_growthRate = in["m_growthRate"].as<float>();
  ...
  if (in["m_phototropism"])
    m_phototropism = in["m_phototropism"].as<float>();
  if (in["m_horizontalTropism"])
    m_horizontalTropism = in["m_horizontalTropism"].as<float>();
  if (in["m_gravityBendingStrength"])
    m_gravityBendingStrength = in["m_gravityBendingStrength"].as<float>();
  if (in["m_gravityBendingThicknessFactor"])
    m_gravityBendingThicknessFactor = in["m_gravityBendingThicknessFactor"].as<float>();
  if (in["m_gravityBendingMax"])
    m_gravityBendingMax = in["m_gravityBendingMax"].as<float>();

  if (in["m_internodeLength"])
    m_internodeLength = in["m_internodeLength"].as<float>();
  if (in["m_internodeLengthThicknessFactor"])
    m_internodeLengthThicknessFactor = in["m_internodeLengthThicknessFactor"].as<float>();
  if (in["m_endNodeThickness"])
    m_endNodeThickness = in["m_endNodeThickness"].as<float>();
  if (in["m_thicknessAccumulationFactor"])
    m_thicknessAccumulationFactor = in["m_thicknessAccumulationFactor"].as<float>();
  if (in["m_thicknessAgeFactor"])
    m_thicknessAgeFactor = in["m_thicknessAgeFactor"].as<float>();
  if (in["m_internodeShadowFactor"])
    m_internodeShadowFactor = in["m_internodeShadowFactor"].as<float>();

  if (in["m_lateralBudCount"])
    m_lateralBudCount = in["m_lateralBudCount"].as<int>();
  if (in["m_maxOrder"])
    m_maxOrder = in["m_maxOrder"].as<int>();
  if (in["m_apicalBudExtinctionRate"])
    m_apicalBudExtinctionRate = in["m_apicalBudExtinctionRate"].as<float>();
  if (in["m_lateralBudFlushingRate"])
    m_lateralBudFlushingRate = in["m_lateralBudFlushingRate"].as<float>();
  if (in["m_apicalControl"])
    m_apicalControl = in["m_apicalControl"].as<float>();
  if (in["m_rootDistanceControl"])
    m_rootDistanceControl = in["m_rootDistanceControl"].as<float>();
  if (in["m_heightControl"])
    m_heightControl = in["m_heightControl"].as<float>();

  if (in["m_apicalDominance"])
    m_apicalDominance = in["m_apicalDominance"].as<float>();
  if (in["m_apicalDominanceLoss"])
    m_apicalDominanceLoss = in["m_apicalDominanceLoss"].as<float>();

  if (in["m_trunkProtection"])
    m_trunkProtection = in["m_trunkProtection"].as<bool>();
  if (in["m_maxFlowLength"])
    m_maxFlowLength = in["m_maxFlowLength"].as<int>();
  if (in["m_lightPruningFactor"])
    m_lightPruningFactor = in["m_lightPruningFactor"].as<float>();
  if (in["m_branchStrength"])
    m_branchStrength = in["m_branchStrength"].as<float>();
  if (in["m_branchStrengthThicknessFactor"])
    m_branchStrengthThicknessFactor = in["m_branchStrengthThicknessFactor"].as<float>();
  if (in["m_branchStrengthLightingThreshold"])
    m_branchStrengthLightingThreshold = in["m_branchStrengthLightingThreshold"].as<float>();
  if (in["m_branchStrengthLightingLoss"])
    m_branchStrengthLightingLoss = in["m_branchStrengthLightingLoss"].as<float>();
  if (in["m_branchBreakingFactor"])
    m_branchBreakingFactor = in["m_branchBreakingFactor"].as<float>();
  if (in["m_branchBreakingMultiplier"])
    m_branchBreakingMultiplier = in["m_branchBreakingMultiplier"].as<float>();

  if (in["m_leafFlushingLightingRequirement"])
    m_leafFlushingLightingRequirement = in["m_leafFlushingLightingRequirement"].as<float>();
  if (in["m_leafFallProbability"])
    m_leafFallProbability = in["m_leafFallProbability"].as<float>();
  if (in["m_leafDistanceToBranchEndLimit"])
    m_leafDistanceToBranchEndLimit = in["m_leafDistanceToBranchEndLimit"].as<float>();

  // Structure
  if (in["m_fruitFlushingLightingRequirement"])
    m_fruitFlushingLightingRequirement = in["m_fruitFlushingLightingRequirement"].as<float>();
  if (in["m_fruitFallProbability"])
    m_fruitFallProbability = in["m_fruitFallProbability"].as<float>();
  if (in["m_fruitDistanceToBranchEndLimit"])
    m_fruitDistanceToBranchEndLimit = in["m_fruitDistanceToBranchEndLimit"].as<float>();
...
bool ShootDescriptor::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  ...
  if (ImGui::TreeNodeEx("Internode", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    changed =
        ImGui::DragFloat2("Base apical angle mean/var", &m_baseNodeApicalAngleMeanVariance.x, 0.1f, 0.0f, 100.0f) ||
        changed;
    ...
    if (ImGui::TreeNodeEx("Angles")) {
      changed =
          ImGui::DragFloat2("Branching angle base/var", &m_branchingAngleMeanVariance.x, 0.1f, 0.0f, 100.0f) || changed;
    ...
    changed =
        ImGui::DragFloat("Internode length thickness factor", &m_internodeLengthThicknessFactor, 0.0001f, 0.0f, 1.0f) ||
        changed;
    changed =
        ImGui::DragFloat3("Thickness min/factor/age", &m_endNodeThickness, 0.0001f, 0.0f, 1.0f, "%.6f") || changed;

    changed = ImGui::DragFloat("Bending strength", &m_gravityBendingStrength, 0.01f, 0.0f, 1.0f, "%.3f") || changed;
    changed =
        ImGui::DragFloat("Bending thickness factor", &m_gravityBendingThicknessFactor, 0.1f, 0.0f, 10.f, "%.3f") ||
        changed;
  ...
  if (ImGui::TreeNodeEx("Bud fate", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    changed = ImGui::DragFloat("Apical bud extinction rate", &m_apicalBudExtinctionRate, 0.01f, 0.0f, 1.0f, "%.5f") ||
              changed;
    changed =
        ImGui::DragFloat("Lateral bud flushing rate", &m_lateralBudFlushingRate, 0.01f, 0.0f, 1.0f, "%.5f") || changed;
  ...
  if (ImGui::TreeNodeEx("Pruning", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    changed =
        ImGui::DragFloat("Branch strength thickness factor", &m_branchStrengthThicknessFactor, 0.01f, 0.0f) || changed;
    changed =
        ImGui::DragFloat("Branch strength lighting threshold", &m_branchStrengthLightingThreshold, 0.01f, 0.0f, 1.0f) ||
        changed;
    changed =
        ImGui::DragFloat("Branch strength lighting loss", &m_branchStrengthLightingLoss, 0.01f, 0.0f, 1.0f) || changed;
    changed =
        ImGui::DragFloat("Branch breaking multiplier", &m_branchBreakingMultiplier, 0.01f, 0.01f, 10.0f) || changed;
  ...
  if (ImGui::TreeNodeEx("Leaf")) {
    changed =
        ImGui::DragFloat("Lighting requirement", &m_leafFlushingLightingRequirement, 0.01f, 0.0f, 1.0f) || changed;
  ...
  if (ImGui::TreeNodeEx("Fruit")) {
    changed =
        ImGui::DragFloat("Lighting requirement", &m_fruitFlushingLightingRequirement, 0.01f, 0.0f, 1.0f) || changed;
  ...
  editor_layer->DragAndDropButton<Material>(m_barkMaterial, "Bark Material##SBS");

  return changed;
}

void ShootDescriptor::CollectAssetRef(std::vector<AssetRef>& list) {
  if (m_rollAngle.Get<ProceduralNoise2D>())
    list.push_back(m_rollAngle);
  if (m_apicalAngle.Get<ProceduralNoise2D>())
    list.push_back(m_apicalAngle);

  if (m_barkMaterial.Get<Material>())
    list.push_back(m_barkMaterial);
}
