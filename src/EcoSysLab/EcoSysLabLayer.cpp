...
void EcoSysLabLayer::UpdateFlows(const std::vector<Entity>* treeEntities, const std::shared_ptr<Strands>& branchStrands, const std::shared_ptr<Strands>& rootStrands, const std::shared_ptr<Strands>& fineRootStrands, int targetTreeIndex)
{
	{
		...
		m_fineRootPoints.resize(fineRootLastStartIndex * 8);
		{
			...
			for (int i = 0; i < rootNodeList.size(); i++)
			{
				...
				m_fineRootSegments[fineRootStartIndex * 5 + fineRootIndex * 5 + 3] = fineRootStartIndex * 8 + fineRootIndex * 8 + 3;
				m_fineRootSegments[fineRootStartIndex * 5 + fineRootIndex * 5 + 4] = fineRootStartIndex * 8 + fineRootIndex * 8 + 4;
			...
			for (const auto& internodeHandle : internodeList)
			{
				...
				for (const auto& bud : internodeData.m_buds)
				{
					...
					else if (bud.m_type == BudType::Fruit)
					{
						if (!isSelected)
						{
							fruitMatrices[fruitStartIndex + fruitIndex] = entityGlobalTransform.m_value * bud.m_reproductiveModule.m_transform;
