...
namespace joescan {
	 ...
	 */
	inline jsError jsSetupConfigParse(
		...
		try {
			...
			for (json::iterator it = jsubroot.begin(); it != jsubroot.end(); ++it) {
					...
					else
					{
						...
						if (_IsLasHead(type)) {
							r = jsScanHeadSetPolygonWindowLaser(
								head, laser, vertices.data(), vertices.size());
						}
						else {
							r = jsScanHeadSetPolygonWindowCamera(
								head, camera, vertices.data(), vertices.size());
						}
				...
				for (json::iterator itc = jchild.begin(); itc != jchild.end(); ++itc) {
					...
					{
						logger(JS_ERROR_NONE,
							head_str + " ExcludedRegions empty!");
					}
					else {
