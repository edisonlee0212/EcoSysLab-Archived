...
void outputGraph(Graph& g, std::string filename, const std::vector<PipeHandle>& pipesInPrevious)
{
	...
	for (Graph::Edge& e : g.m_edges)
	{
		...
		file << "set arrow from " << g[Graph::source(e)].x << "," << g[Graph::source(e)].y
			<< " to " << g[Graph::target(e)].x << "," << g[Graph::target(e)].y << " as 1\n";
	}
