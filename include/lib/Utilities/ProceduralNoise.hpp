...
namespace EcoSysLab
{
	...
	void ProceduralNoiseNodeData<T>::Deserialize(const YAML::Node& in)
	{
		...
		if (in["m_valueType"]) m_valueType = static_cast<ProceduralNoiseValueType>(in["m_valueType"].as<unsigned>());
		if (in["m_frequency"]) m_frequency = in["m_frequency"].as<T>();
		if (in["m_constantValue"]) m_constantValue = in["m_constantValue"].as<float>();
		if (in["m_offset"]) m_offset = in["m_offset"].as<T>();
	}
