...
template <typename T>
void Physics2D<T>::Update(const std::function<void(RigidBody2D<T>& collisionRigidBody)>& modifyRigidBodyFunc) {
...
void Physics2D<T>::OnInspect(const std::function<void(glm::vec2 position)>& func,
  ...
  for (const auto& particle : m_rigidBodies2D) {
    ...
    drawList->AddCircleFilled(
        canvasPosition, glm::clamp(zoomFactor * pointRadius, 1.0f, 100.0f),
        IM_COL32(255.0f * pointColor.x, 255.0f * pointColor.y, 255.0f * pointColor.z, 255.0f * pointColor.w));
  }

  drawList->AddCircle(origin, glm::clamp(0.5f * zoomFactor, 1.0f, 100.0f), IM_COL32(255, 0, 0, 255));
