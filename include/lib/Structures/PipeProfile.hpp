...
#pragma endregion
	...
	public:
		...
		[[nodiscard]] glm::vec2 FindAvailablePosition(const glm::vec2& startPosition, const glm::vec2& moveStep);
		ProfileData m_data = {};
		ProfileInfo m_info = {};
		[[nodiscard]] bool IsRecycled() const;
		...
		void Recenter();
		void RegulateBoundary(float targetSegmentLength);
	...
	bool PipeProfile<ProfileData, CellData>::OnInspect(const bool editable)
	{
		...
		if (editable && isMouseHovered && !m_addingLine && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
			m_info.m_boundary.m_points.clear();
			m_info.m_boundary.m_points.emplace_back(mousePosInCanvas.x, mousePosInCanvas.y);
			m_addingLine = true;
		}
		...
		drawList->AddCircle(origin,
			...
			IM_COL32(255,
				0,
				0, 255));
		if (m_info.m_boundary.m_points.size() >= 2) {
			for (int i = 0; i < m_info.m_boundary.m_points.size() - 1; i++) {
				drawList->AddLine(ImVec2(origin.x + m_info.m_boundary.m_points[i].x * zoomFactor,
					origin.y + m_info.m_boundary.m_points[i].y * zoomFactor),
					ImVec2(origin.x + m_info.m_boundary.m_points[i + 1].x * zoomFactor,
						origin.y + m_info.m_boundary.m_points[i + 1].y * zoomFactor),
					IM_COL32(255, 0, 0, 255), 2.0f);
			}
		}
		if (m_addingLine) {
			if (glm::distance(m_info.m_boundary.m_points.back(), { mousePosInCanvas.x, mousePosInCanvas.y }) >= 10.0f / zoomFactor)
				m_info.m_boundary.m_points.emplace_back(mousePosInCanvas.x, mousePosInCanvas.y);
		...
		{
			for (int i = 0; i < m_info.m_boundary.m_points.size(); i++)
			{
				drawList->AddLine(ImVec2(origin.x + m_info.m_boundary.m_points[m_info.m_boundary.m_points.size() - 1].x * zoomFactor,
					origin.y + m_info.m_boundary.m_points[m_info.m_boundary.m_points.size() - 1].y * zoomFactor),
					ImVec2(origin.x + m_info.m_boundary.m_points[0].x * zoomFactor,
						origin.y + m_info.m_boundary.m_points[0].y * zoomFactor),
					IM_COL32(255, 0, 0, 255), 2.0f);
			}
		}
		...
		const glm::vec2& moveStep)
	{
		...
		while(!found)
		{
			...
			{
				if(glm::distance(i.m_info.m_offset, retVal) < 2.05f * m_info.m_cellRadius)
				{
					found = false;
					break;
				}
			}
			if(!found) retVal += moveStep;
		}
		return retVal;
	}
	...
	void PipeProfile<ProfileData, CellData>::Recenter()
	{
		...
		for (const auto& i : m_info.m_boundary.m_points) sum += i;
		sum /= m_info.m_boundary.m_points.size();
		for (auto& i : m_info.m_boundary.m_points)
		{
			i -= sum;
		}
	}
	...
	template <typename ProfileData, typename CellData>
	void PipeProfile<ProfileData, CellData>::RegulateBoundary(float targetSegmentLength)
	{
		//Increment
		for(int i = 0; i < m_info.m_boundary.m_points.size(); i++)
		{
			const auto distance = glm::distance(m_info.m_boundary.m_points[i], m_info.m_boundary.m_points[(i + 1) % m_info.m_boundary.m_points.size()]);
			if(distance > 2.0f * targetSegmentLength)
			{
				m_info.m_boundary.m_points.insert(m_info.m_boundary.m_points.begin() + i, (m_info.m_boundary.m_points[i] + m_info.m_boundary.m_points[(i + 1) % m_info.m_boundary.m_points.size()]) / 2.0f);
			}
		}
		//Simplification
		for (int i = 0; i < m_info.m_boundary.m_points.size(); i++)
	...
	template <typename ProfileData, typename CellData>
	void PipeProfile<ProfileData, CellData>::GenerateContours()
	{
	}
	...
	void PipeProfile<ProfileData, CellData>::FillCells()
	{
		...
		for (const auto& i : m_cells)
		{
 		...
 		for (const auto& i : m_info.m_boundary.m_points)
		{
			min = glm::min(min, i);
			max = glm::max(i, max);
		}
